...
import (
	...
	kubernetesutils "github.com/codeamp/circuit/plugins/kubernetes"
...
func (x *LoadBalancers) doLoadBalancer(e transistor.Event) error {
	...
	kubeconfig, err := kubernetesutils.SetupKubeConfig(e, configPrefix)
	if err != nil {
		log.Info(err.Error())
		return err
	}
	...
	if err != nil {
		...
		x.events <- kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("failed"), failMessage, err)
		return err
	}
	...
	deploymentName := kubernetesutils.GenDeploymentName(projectSlug, svcName.GetString())

	var serviceType v1.ServiceType
	var servicePorts []v1.ServicePort
	serviceAnnotations := make(map[string]string)
	namespace := kubernetesutils.GenNamespaceName(payload.Environment, projectSlug)
	createNamespaceErr := kubernetesutils.CreateNamespaceIfNotExists(namespace, coreInterface)
	if createNamespaceErr != nil {
		x.events <- kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("failed"), createNamespaceErr.Error(), err)
		return nil
	}
	...
	for _, p := range listenerPairs.GetStringSlice() {
		...
		if err != nil {
			x.events <- kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("failed"), err.Error(), err)
			return nil
		}
	...
	case err == nil:
		...
		if err != nil {
			...
			x.events <- kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("failed"), errMsg, err)
			return nil
		}
	...
	if lbType == plugins.GetType("external") || lbType == plugins.GetType("office") {
		...
		event = kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("waiting"), "waiting for route53", nil)
		...
		ELBDNS = fmt.Sprintf("%s.%s", lbName.GetString(), kubernetesutils.GenNamespaceName(payload.Environment, projectSlug))
		event = kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("complete"), "complete", nil)
	}
...
func (x *LoadBalancers) doDeleteLoadBalancer(e transistor.Event) error {
	...
	if svcGetErr == nil {
		...
		x.events <- kubernetesutils.CreateProjectExtensionEvent(e, plugins.GetAction("status"), plugins.GetState("deleted"), "", nil)
	} else {
